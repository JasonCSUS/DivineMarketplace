...
public final class SQLiteMarketIndexStore {
    ...
    public void adminReplaceAll(Collection<FlattenedMarketIndexEntry> entries) {
    ...
    /** Returns a queued PUT mutation for a single entry — does not write to SQLite. */
    public SQLiteMutation putMutation(FlattenedMarketIndexEntry entry) {
        return SQLiteMutation.put(TABLE, entry.marketKey(), encode(entry));
    }
