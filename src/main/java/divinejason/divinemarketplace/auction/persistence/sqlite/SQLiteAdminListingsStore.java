...
/*
 * File role: Persists and queries admin listings records in SQLite while exposing size/retention helpers where needed.
 */
...
public final class SQLiteAdminListingsStore {
    private static final String TABLE = "admin_listings";

    private final SQLiteStore sqliteStore;
    private final Map<String, AdminTransactionRecord> cacheById = new LinkedHashMap<>();
    private final Object lock = new Object();

    public SQLiteAdminListingsStore(SQLiteStore sqliteStore) {
    ...
    public int purgeOldestIfOverMaxSize() {
        return purgeOldestUntilUnderBytes(ConfigService.get().adminListingsHistoryMaxBytes());
    }
